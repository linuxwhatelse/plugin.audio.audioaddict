...
class AudioAddict:
    ...
    @property
    def _cache(self):
        return self.__cache.get(self._cache_file) or self._read_cache(self._cache_file)

    @property
    def _ccache(self):
        return self.__cache.get(self._ccache_file) or self._read_cache(
            self._ccache_file
        )
    ...
    def _get(self, *paths, **kwargs):
        return self._api_call(requests.get, paths, auth=("mobile", "apps"), **kwargs)
    ...
    def _post(self, *paths, **kwargs):
        ...
        return self._api_call(
            requests.post, paths, auth=("mobile", "apps"), cache=cache, **kwargs
        )
    ...
    def _delete(self, *paths, **kwargs):
        ...
        if cache:
            kwargs.pop("cache")
        return self._api_call(
            requests.delete, paths, auth=("mobile", "apps"), cache=cache, **kwargs
        )
    ...
    def next_channel_track(
        self, channel, tune_in=True, refresh=False, pop=False, live=True
    ):
        is_live = False
        track = None
        ...
        if live:
            ...
            for show in self.get_live_shows():
                channels = [
                    c
                    for c in show.get("show", {}).get("channels", [])
        ...
        if not track:
            ...
            track_list = self.get_track_list(channel, refresh=refresh, cache=cache_file)

            is_new = False
            if (
                track_list.get("channel_id") != channel_id
    ...
    def next_playlist_track(self, playlist_id, refresh=False, pop=True):
        ...
        track_list = self.get_playlist_tracks(
            playlist_id, refresh=refresh, cache=cache_file
        )

        is_new = False

        if (
            track_list.get("id") != playlist_id
    ...
    def get_favorite_channels(self, refresh=False):
        ids = [f.get("channel_id") for f in self.get_favorites(refresh=refresh)]
    ...
    def get_subscriptions(self):
        return self._get(
            "members", self.member_id, "subscriptions", "active", cache=None
        )

    def get_channel_filters(self, refresh=False):
        return self._get("channel_filters", cache_time=24 * 60, refresh=refresh)
    ...
    def get_shows(self, channel, page=1, per_page=25, refresh=False):
        ...
        if not field:
            raise ValueError(
                'Unable to determin a valid field for channel "{}"'.format(channel)
            )
        ...
        return self._get(
            "shows", cache_key=cache_key, cache_time=60, refresh=refresh, **query
        ).get("results", [])
    ...
    def search_channels(self, query, page=1, per_page=25):
        ...
        return self._get("channels", cache_key=cache_key, cache_time=10, **query)
    ...
    def get_playlists(self, order_by, page=1, per_page=25):
        ...
        return self._get("playlists", cache_key=cache_key, cache_time=10, **query)
    ...
    def follow_show(self, show):
        resp = self._post("members", self.member_id, "followed_items", "show", show)
    ...
    def preferred_quality(self, quality_id):
        return self._post(
            "members", self.member_id, "preferred_quality", quality_id=quality_id
        )
    ...
    def unfollow_show(self, show):
        resp = self._delete("members", self.member_id, "followed_items", "show", show)
