...
def list_networks():
    ...
    for key, data in addict.NETWORKS.items():
        ...
        item.setArt(
            {
                "thumb": os.path.join(ADDON_DIR, "resources", "assets", key + ".png"),
            }
        )
        ...
        item.addContextMenuItems(
            [
                (
                    utils.translate(30307),
                    "RunPlugin({})".format(utils.build_path("refresh", key)),
                ),
            ],
            True,
        )
...
def list_network(network):
    ...
    items.append(
        (
            utils.build_path("channels", network),
            xbmcgui.ListItem(utils.translate(30321)),
            True,
        )
    )
    ...
    if aa.network["has_shows"]:
        items.append(
            (
                utils.build_path("shows", network),
                xbmcgui.ListItem(utils.translate(30322)),
                True,
            )
        )
        ...
        items.append(
            (
                utils.build_path("shows", network, "schedule"),
                xbmcgui.ListItem(utils.translate(30332)),
                True,
            )
        )
    ...
    if aa.network["has_playlists"]:
        items.append(
            (
                utils.build_path("playlists", network),
                xbmcgui.ListItem(utils.translate(30338)),
                True,
            )
        )
    ...
    items.append(
        (
            utils.build_path("search", network),
            xbmcgui.ListItem(utils.translate(30323)),
            True,
        )
    )
...
def list_styles(network):
    ...
    for style in filters:
        item = xbmcgui.ListItem(
            "{} ({})".format(_enc(style.get("name")), len(style.get("channels", [])))
        )
        items.append(
            (utils.build_path("channels", network, style.get("key")), item, True)
        )
...
def list_channels(network, style=None, channels=None, do_list=True):
    ...
    for channel in channels:
        item_url = utils.build_path("play", "channel", network, channel.get("key"))
        ...
        cmenu.append(
            (
                utils.translate(30330),
                "Container.Update({}, return)".format(
                    utils.build_path("listen_history", network, channel.get("key"))
                ),
            )
        )
...
def list_shows_menu(network):
    ...
    items.append(
        (
            utils.build_path("shows", network, "followed"),
            xbmcgui.ListItem(utils.translate(30324)),
            True,
        )
    )
    ...
    items.append(
        (
            utils.build_path("shows", network, "fields", "channel_filter_name"),
            xbmcgui.ListItem(utils.translate(30325)),
            True,
        )
    )
    ...
    items.append(
        (
            utils.build_path("shows", network, "fields", "channel_name"),
            xbmcgui.ListItem(utils.translate(30316)),
            True,
        )
    )
...
def list_shows_styles(network, field):
    ...
    for facet in facets:
        ...
        item_url = utils.build_path("shows", network, "facets", facet.get("name"))
        items.append((item_url, xbmcgui.ListItem(_enc(facet.get("label"))), True))
...
def list_shows_schedule(network, page=1):
    ...
    for show in shows:
        ...
        if show.get("now_playing", False):
            ...
            if active.get("is_live", False) and active.get("channel") == channel.get(
                "key"
            ):
                item.select(True)
        else:
...
def list_playlist_menu(network):
    ...
    items.append(
        (
            utils.build_path("playlists", network, "popular"),
            xbmcgui.ListItem(utils.translate(30339)),
            True,
        )
    )
    ...
    items.append(
        (
            utils.build_path("playlists", network, "newest"),
            xbmcgui.ListItem(utils.translate(30340)),
            True,
        )
    )
    ...
    items.append(
        (
            utils.build_path("playlists", network, "followed"),
            xbmcgui.ListItem(utils.translate(30341)),
            True,
        )
    )
...
def search(network, filter_=None, query=None, page=1):
    ...
    if not filter_:
        ...
        items.append(
            (
                utils.build_path("search", network, "channels", query),
                xbmcgui.ListItem(utils.translate(30321)),
                True,
            )
        )
        ...
        items.append(
            (
                utils.build_path("search", network, "shows", query),
                xbmcgui.ListItem(utils.translate(30322)),
                True,
            )
        )
...
def play_channel(network, channel, live=False):
    ...
    with utils.busy_dialog():
        is_live, track = aa.next_channel_track(
            channel, tune_in=True, refresh=True, pop=False, live=live
        )

        utils.logd(
            "Activating first track: {}, is-live: {}".format(track.get("id"), is_live)
        )
        item_url = utils.build_path(
            "channel", "track", network, channel, track.get("id"), is_live=is_live
        )
...
@MPR.s_url(
    "/channel/track/<network>/<channel>/<track_id>/",
    type_cast={"track_id": int, "is_live": bool},
)
def resolve_channel_track(network, channel, track_id, is_live=False):
    utils.logd("Resolving track:", track_id)
    aa = addict.AudioAddict.get(PROFILE_DIR, network)

    current_is_live, track = aa.next_channel_track(
        channel, tune_in=False, refresh=False, pop=True, live=is_live
    )

    utils.logd(
        "Resolved track: {}, is-live: {}".format(track.get("id"), current_is_live)
    )

    if track_id != track.get("id"):
        utils.logw(
            "Got unexpected track from cache! "
    ...
    is_live, track = aa.next_channel_track(
        channel, tune_in=False, refresh=False, pop=False, live=not current_is_live
    )
    ...
    utils.logd("Queuing track: {}, is-live: {}".format(track.get("id"), is_live))
    playlist.add(item.getPath(), item)
...
@MPR.s_url("/play/playlist/<network>/<playlist_id>/", type_cast={"playlist_id": int})
...
@MPR.s_url(
    "/playlist/track/<network>/<playlist_id>/<track_id>",
    type_cast={"playlist_id": int, "track_id": int},
)
...
def setup(notice=True, update_cache=False):
    ...
    if not aa.login(username, password):
        if xbmcgui.Dialog().yesno(utils.translate(30309), utils.translate(30310)):
